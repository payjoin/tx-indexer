use crate::{
    ScriptPubkeyHash, abstract_fingerprints::HasNLockTime, abstract_id::{AbstractId, LooseIds},
    abstract_types::{
    ...
    graph_index::{IndexedGraph, TxHandleLike, TxInHandleLike, TxOutHandleLike},
...
pub struct TxHandle<'a> {
    id: TxId,
    index: &'a (dyn IndexedGraph<LooseIds> + 'a),
}

impl<'a> TxHandle<'a> {
    pub fn new(id: TxId, index: &'a (dyn IndexedGraph<LooseIds> + 'a)) -> Self {
        Self { id, index }
    }
...
impl AbstractTransaction for TxHandle<'_> {
    ...
    fn inputs(
        ...
        dyn Iterator<Item = Box<dyn AbstractTxIn<Id = Self::Id>>> + '_,
    > {
        let tx = self
            .index
            .tx(&self.id)
        ...
        let input_boxes: Vec<Box<dyn AbstractTxIn<Id = Self::Id>>> =
            tx.inputs().collect();
...
impl<'a> TxHandleLike<LooseIds> for TxHandle<'a> {
    fn output_handles(&self) -> Box<dyn Iterator<Item = Box<dyn TxOutHandleLike<LooseIds> + '_>> + '_> {
        let handles: Vec<Box<dyn TxOutHandleLike<LooseIds> + 'a>> = self
            .outputs()
            .map(|h| Box::new(h) as Box<dyn TxOutHandleLike<LooseIds> + 'a>)
            .collect();
        Box::new(handles.into_iter())
    }

    fn spent_coins(&self) -> Box<dyn Iterator<Item = <LooseIds as AbstractId>::TxOutId> + '_> {
        Box::new(EnumerateSpentTxOuts::spent_coins(self))
    }
...
impl<'a> TxInHandle<'a> {
    pub fn new(id: TxInId, index: &'a (dyn IndexedGraph<LooseIds> + 'a)) -> Self {
        Self { id, index }
    }
...
impl<'a> TxInHandleLike<LooseIds> for TxInHandle<'a> {
    fn tx(&self) -> Box<dyn TxHandleLike<LooseIds> + '_> {
        Box::new(self.tx())
    }
}
...
impl<'a> TxOutHandle<'a> {
    ...
    pub fn new(id: TxOutId, index: &'a (dyn IndexedGraph<LooseIds> + 'a)) -> Self {
        Self { id, index }
    }
...
impl<'a> TxOutHandleLike<LooseIds> for TxOutHandle<'a> {
    fn id(&self) -> <LooseIds as AbstractId>::TxOutId {
        self.id
    }
    ...
    fn spent_by(&self) -> Option<Box<dyn TxInHandleLike<LooseIds> + '_>> {
        self.spent_by()
            .map(|h| Box::new(h) as Box<dyn TxInHandleLike<LooseIds>>)
    }
}
