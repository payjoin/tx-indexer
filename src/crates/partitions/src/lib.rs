...
mod tests {
    ...
    impl AbstractTransaction for MockTx {
        ...
        fn output_at(&self, _: usize) -> Option<Box<dyn AbstractTxOut + '_>> {
            None
        }
    ...
    impl EnumerateOutputValueInArbitraryOrder for MockTx {
        fn output_values(&self) -> impl Iterator<Item = Amount> {
            self.outputs.iter().map(|&v| Amount::from_sat(v))
        }
    }
    ...
    fn figure_2_unmixed() {
        let tx = MockTx {
            ...
            outputs: vec![25, 8, 50, 14],
        };
        let mappings = get_non_derived_mappings(&tx);
        assert_eq!(mappings.len(), 1);
        assert!(is_mapping_valid(&mappings[0], &tx.inputs, &tx.outputs));
    ...
    fn figure_6_mixed() {
        let tx = MockTx {
            inputs: vec![21, 12, 36, 28],
            outputs: vec![25, 8, 31, 19, 14],
        };
        let mappings = get_non_derived_mappings(&tx);
        assert_eq!(mappings.len(), 2);
        for m in &mappings {
            assert!(is_mapping_valid(m, &tx.inputs, &tx.outputs));
        }
    }
